...
!mal Include outputs associated with the Tlcds   

! NOTE: The following lines of code were generated by a Matlab script called "Write_ChckOutLst.m"
!      using the parameters listed in the "OutListParameters.xlsx" Excel file. Any changes to these 
!      lines should be modified in the Matlab script and/or Excel worksheet as necessary. 
! This code was generated by Write_ChckOutLst.m at 30-Jan-2012 12:30:08.
   CHARACTER( 9),PARAMETER  :: ValidParamAry(1116) =  (/ &                         ! This lists the names of the allowed parameters, which must be sorted alphabetically
                               ...
                               "Q_TFA1   ","Q_TFA2   ","Q_TFRL   ",                                                 &
                               "Q_TLCDX  ","Q_TLCDY  ",                                                             &
                               "Q_TSS1   ","Q_TSS2   ","Q_Y      ","Q_YAW    ",                                     &
   ...
   INTEGER,      PARAMETER  :: ParamIndxAry(1116) =  (/ &                          ! This lists the index into AllOuts(:) of the allowed parameters ValidParamAry(:)
                                 Anch1Ang ,  Anch1Ten ,  Anch2Ang ,  Anch2Ten ,  Anch3Ang ,  Anch3Ten ,  Anch4Ang , &
                                 Anch4Ten ,  Anch5Ang ,  Anch5Ten ,  Anch6Ang ,  Anch6Ten ,  Anch7Ang ,  Anch7Ten , &
   ...
   CHARACTER(10),PARAMETER  :: ParamUnitsAry(1116) =  (/ &                         ! This lists the units corresponding to the allowed parameters
                               ...
                               "(kW)      ","(rpm)     ","(kN穖)    ","(m/s)     ","(deg)     ","(kN穖)    ","(deg/s^2) ", &
                               "(-)       ","(-)       ","(kW)      ","(kN穖)    ","(rpm)     ","(m)       ","(m)       ", &
                               "(m)       ","(deg/s^2) ","(deg/s^2) ","(deg/s^2) ","(kN)      ","(kN)      ","(kN)      ", &
                               "(kN)      ","(kN)      ","(kN)      ","(kN穖)    ","(kN穖)    ","(kN穖)    ","(kN穖)    ", &
                               "(kN穖)    ","(kN穖)    ","(deg)     ","(deg)     ","(deg)     ","(rpm)     ","(rpm)     ", &
                               "(rpm)     ","(-)       ","(-)       ","(-)       ","(kN)      ","(kN)      ","(kN)      ", &
                               "(kN)      ","(kN)      ","(kN)      ","(kN穖)    ","(kN穖)    ","(kW)      ","(kN穖)    ", &
                               "(deg/s^2) ","(deg/s^2) ","(deg/s^2) ","(kN穖)    ","(kN穖)    ","(kN穖)    ","(kN穖)    ", &
                               ...
                               "(m/s^2)   ","(rad/s^2) ",                                                                  &
                               "(m/s^2)   ","(m/s^2)   ",                                                                  & 
                               "(m/s^2)   ","(m/s^2)   ","(rad/s^2) ","(rad/s^2) ","(m/s)     ",                           &
                               "(m/s)     ","(m/s)     ","(m/s)     ","(m/s)     ","(m/s)     ","(m/s)     ","(m/s)     ", &
                               "(m/s)     ","(rad/s)   ","(rad/s)   ","(m/s)     ","(rad/s)   ","(rad/s)   ","(rad/s)   ", &
                               "(m/s)     ","(m/s)     ","(rad/s)   ","(m/s)     ","(m/s)     ","(rad/s)   ",              &
                               "(m/s)     ","(m/s)     ",                                                                  &
                               "(m/s)     ",                                                                               &
                               ...
                               "(kN)      ","(kN穖)    ","(kN穖)    ","(kN穖)    ","(deg)     ","(deg)     ","(deg)     ", &
                               "(deg)     ","(deg)     ","(deg)     ","(m)       ","(m)       ","(m)       ","(m)       ", &
                               "(m)       ","(m)       ","(kN)      ","(kN)      ","(kN)      ","(kN穖)    ","(kN穖)    ", &
                               "(kN穖)    ","(m)       ","(m)       ","(m)       ","(deg)     ","(deg)     ","(deg)     ", &
...
!yus Use the Tlcd module as well
USE                             Tlcd
...
!yus Start of proposed change.  1-Dec-2014.
!yus Include TlcdX and TlcdY control switches and time to start control.

   ! TlcdXCMode - TlcdX control mode.

CALL ReadIVar ( UnIn, PriFile, TlcdXCMode, 'TlcdXCMode', 'TlcdX control mode' )

IF ( ( .NOT. Cmpl4SFun ) .AND. ( TlcdXCMode == 2 ) )  THEN
   CALL Abort ( ' TlcdXCMode can only equal 2 when FAST is interfaced with Simulink.'// &
                '  Set TlcdXCMode to 0 or 1 or interface FAST with Simulink.'             )
ELSEIF ( ( TlcdXCMode < 0 ) .OR. ( TlcdXCMode > 2 ) )  THEN
   CALL Abort ( ' TlcdXCMode must be 0, 1, or 2.' )
ENDIF


   ! TTlcdXCOn - Time to enable TlcdX control.

CALL ReadRVar ( UnIn, PriFile, TTlcdXCOn, 'TTlcdXCOn', 'Time to enable TlcdX control' )

IF ( Cmpl4SFun .AND. ( TlcdXCMode == 2 ) .AND. ( TTlcdXCOn /= 0.0 ) )  THEN
   CALL Abort ( ' TlcdX control must be enabled at time zero when implemented in Simulink.'//      &
                '  Set TTlcdXCOn to 0.0, set TlcdXCMode to 0 or 1, or use the standard version of FAST.'   )
ELSEIF ( TTlcdXCOn < 0.0 )  THEN
   CALL Abort ( ' TTlcdXCOn must not be negative.' )
ENDIF


   ! TlcdYCMode - TlcdY control mode.

CALL ReadIVar ( UnIn, PriFile, TlcdYCMode, 'TlcdYCMode', 'TlcdY control mode' )

IF ( ( .NOT. Cmpl4SFun ) .AND. ( TlcdYCMode == 2 ) )  THEN
   CALL Abort ( ' TlcdYCMode can only equal 2 when FAST is interfaced with Simulink.'// &
                '  Set TlcdYCMode to 0 or 1 or interface FAST with Simulink.'             )
ELSEIF ( ( TlcdYCMode < 0 ) .OR. ( TlcdYCMode > 2 ) )  THEN
   CALL Abort ( ' TlcdYCMode must be 0, 1, or 2.' )
ENDIF


   ! TTlcdYCOn - Time to enable TlcdY control.

CALL ReadRVar ( UnIn, PriFile, TTlcdYCOn, 'TTlcdYCOn', 'Time to enable TlcdY control' )

IF ( Cmpl4SFun .AND. ( TlcdYCMode == 2 ) .AND. ( TTlcdYCOn /= 0.0 ) )  THEN
   CALL Abort ( ' TlcdY control must be enabled at time zero when implemented in Simulink.'//      &
                '  Set TTlcdYCOn to 0.0, set TlcdYCMode to 0 or 1, or use the standard version of FAST.'   )
ELSEIF ( TTlcdYCOn < 0.0 )  THEN
   CALL Abort ( ' TTlcdYCOn must not be negative.' )
ENDIF
...
!yus Start of proposed change.   1-Dec-2014.
!yus Include feature flags for TlcdX and TlcdY

   ! TlcdXDOF - TlcdX DOF.

CALL ReadLVar ( UnIn, PriFile, TlcdXDOF, 'TlcdXDOF', 'TlcdX DOF' )


   ! TlcdYDOF - TlcdX DOF.

CALL ReadLVar ( UnIn, PriFile, TlcdYDOF, 'TlcdYDOF', 'TlcdY DOF' )
...
!yus Start of proposed change.  1-Dec-2014.
!yus Initial liquid displacements in the vertical column of TlcdX and TlcdY


   ! TlcdXDsp - TlcdX initial or fixed liquid displacement in the vertical column.

CALL ReadRVar ( UnIn, PriFile, TlcdXDsp, 'TlcdXDsp', 'TlcdX initial or fixed liquid displacement in the vertical column' )


   ! TlcdYDsp - TlcdY initial or fixed liquid displacement in the vertical column.

CALL ReadRVar ( UnIn, PriFile, TlcdYDsp, 'TlcdYDsp', 'TlcdY initial or fixed liquid displacement in the vertical column' )
...
!yus Start of proposed change.  v7.01 1-Dec-2014s.
!yus include the switch for Tlcdx location, either nacelle or platform

   ! TlcdXLoc - TlcdX location.

CALL ReadIVar ( UnIn, PriFile, TlcdXLoc, 'TlcdXLoc', 'TlcdX location' )

IF ( ( TlcdXLoc < 1 ) .OR. ( TlcdXLoc > 2 ) )  THEN
   CALL Abort ( ' TlcdXLoc must be 1 or 2.' )
ENDIF



   ! TlcdXRefxnt - Downwind distance from tower-top or platform ref to TlcdX bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdXRefxnt, 'TlcdXRefxnt', 'Reference Downwind distance from the tower-top (n) or platform reference (t) to the TlcdX bottom plane center' )


   ! TlcdXRefynt - Lateral  distance from tower-top or platform ref to TlcdX bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdXRefynt, 'TlcdXRefynt', 'Reference Lateral  distance from the tower-top (n) or platform reference (t) to the TlcdX bottom plane center' )


   ! TlcdXRefznt - Vertical distance from tower-top or platform ref to TlcdX bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdXRefznt, 'TlcdXRefznt', 'Reference Vertical distance from the tower-top (n) or platform reference (t) to the TlcdX bottom plane center' )


   ! TlcdXAngle  - Angle that TlcdX displacement direction is rotated about the vertical axis.

CALL ReadRVar ( UnIn, PriFile, TlcdXAngle, 'TlcdXAngle', 'Angle that TlcdX displacement direction is rotated about the vertical axis' )




   ! TlcdYLoc - TlcdY location.

CALL ReadIVar ( UnIn, PriFile, TlcdYLoc, 'TlcdYLoc', 'TlcdY location' )

IF ( ( TlcdYLoc < 1 ) .OR. ( TlcdYLoc > 2 ) )  THEN
   CALL Abort ( ' TlcdYLoc must be 1 or 2.' )
ENDIF


   ! TlcdYRefxnt - Downwind distance from tower-top or platform ref to TlcdY bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdYRefxnt, 'TlcdYRefxnt', 'Reference Downwind distance from the tower-top (n) or platform reference (t) to the TlcdY bottom plane center' )


   ! TlcdYRefynt - Lateral  distance from tower-top or platform ref to TlcdY bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdYRefynt, 'TlcdYRefynt', 'Reference Lateral  distance from the tower-top (n) or platform reference (t) to the TlcdY bottom plane center' )


   ! TlcdYRefznt - Vertical distance from tower-top or platform ref to TlcdY bottom plane center.

CALL ReadRVar ( UnIn, PriFile, TlcdYRefznt, 'TlcdYRefznt', 'Reference Vertical distance from the tower-top (n) or platform reference (t) to the TlcdY bottom plane center' )


   ! TlcdYAngle  - Angle that TlcdY displacement direction is rotated about the vertical axis.

CALL ReadRVar ( UnIn, PriFile, TlcdYAngle, 'TlcdYAngle', 'Angle that TlcdY displacement direction is rotated about the vertical axis' )


!yus End of proposed change.  v7.01 1-Dec-2014s.
...
!mal Start of proposed change.  v6.10a-mal  1-Mar-2009.
!mal Tlcd mass properties


   ! TlcdXMass - TlcdX mass.

!CALL ReadRVar ( UnIn, PriFile, TlcdXMass, 'TlcdXMass', 'TlcdX mass' )

!IF ( TlcdXMass < 0.0 )  CALL Abort ( ' TlcdXMass must not be negative.' )

!mal Start of proposed change.  v6.10a-mal  14-May-2009.
!mal Make sure mass is non-zero if the DOF is on

!IF (( TlcdXMass == 0.0 ) .AND. (TlcdXDOF))  CALL Abort ( ' TlcdXMass must not be zero if the TlcdXDOF is True.' )

!mal End of proposed change.  v6.10a-mal  14-May-2009.


   ! TlcdYMass - TlcdY mass.

!CALL ReadRVar ( UnIn, PriFile, TlcdYMass, 'TlcdYMass', 'TlcdY mass' )

!IF ( TlcdYMass < 0.0 )  CALL Abort ( ' TlcdYMass must not be negative.' )

!mal Start of proposed change.  v6.10a-mal  14-May-2009.
!mal Make sure mass is non-zero if the DOF is on

!IF (( TlcdYMass == 0.0 ) .AND. (TlcdYDOF))  CALL Abort ( ' TlcdYMass must not be zero if the TlcdYDOF is True.' )
...
!yus Start of proposed change.  v7.01  1-Dec-2014.
!yus Tlcd inputs also change skipcomment to readcom and abort to progabort
...
CALL ReadCom ( UnIn, PriFile, 'Tlcd parameters'  )

    ! TlcdDens - Liquid density in TLCD

CALL ReadRVar ( UnIn, PriFile, TlcdDens, 'TlcdDens', 'Liquid density in TLCD' )

IF ( TlcdDens < 0.0 )  CALL ProgAbort ( ' TlcdDens must not be negative.' )


   ! TlcdXHorArea - TlcdX initial or fixed horizontal cross-section area.

CALL ReadRVar ( UnIn, PriFile, TlcdXHorArea, 'TlcdXHorArea', 'TlcdX initial or fixed horizontal cross-section area' )

IF ( TlcdXHorArea < 0.0 )  CALL ProgAbort ( ' TlcdXHorArea must not be negative.' )


   ! TlcdXVerArea - TlcdX initial or fixed vertical cross-section area.

CALL ReadRVar ( UnIn, PriFile, TlcdXVerArea, 'TlcdXVerArea', 'TlcdX initial or fixed vertical cross-section area' )

IF ( TlcdXVerArea < 0.0 )  CALL ProgAbort ( ' TlcdXVerArea must not be negative.' )



   ! TlcdXLen - TlcdX initial or fixed total length.

CALL ReadRVar ( UnIn, PriFile, TlcdXLen, 'TlcdXLen', 'TlcdX initial or fixed total length' )

IF ( TlcdXLen < 0.0 )  CALL ProgAbort ( ' TlcdXLen must not be negative.' )


   ! TlcdXHorLen - TlcdX initial or fixed horizontal length.

CALL ReadRVar ( UnIn, PriFile, TlcdXHorLen, 'TlcdXHorLen', 'TlcdX initial or fixed horizontal length' )

IF ( TlcdXHorLen < 0.0 )  CALL ProgAbort ( ' TlcdXHorLen must not be negative.' )
IF ( TlcdXHorLen > TlcdXLen )  CALL ProgAbort ( ' TlcdXHorLen must be shorter than TlcdXLen.' )


   ! TlcdXHdLoss - TlcdX Initial or fixed head-loss coefficient.

CALL ReadRVar ( UnIn, PriFile, TlcdXHdLoss, 'TlcdXHdLoss', 'TlcdX Initial or fixed head-loss coefficient' )
IF ( TlcdXHdLoss < 0.0 )  CALL ProgAbort ( 'TlcdXHdLoss must not be negative.' )


 ! TlcdXSSpr - Initial TlcdX stop spring stiffness.

CALL ReadRVar ( UnIn, PriFile, TlcdXSSpr, 'TlcdXSSpr', 'TlcdX initial or fixed stop spring stiffness' )

IF ( TlcdXSSpr < 0.0 )  CALL ProgAbort ( ' TlcdXSSpr must not be negative.' )

   ! TlcdXSDamp - Initial TlcdX stop damping.

CALL ReadRVar ( UnIn, PriFile, TlcdXSDamp, 'TlcdXSDamp', 'TlcdX initial or fixed stop damping' )

IF ( TlcdXSDamp < 0.0 )  CALL ProgAbort ( ' TlcdXSDamp must not be negative.' )


   ! TlcdYHorArea - TlcdY initial or fixed horizontal cross-section area.

CALL ReadRVar ( UnIn, PriFile, TlcdYHorArea, 'TlcdYHorArea', 'TlcdY initial or fixed horizontal cross-section area' )

IF ( TlcdYHorArea < 0.0 )  CALL ProgAbort ( ' TlcdYHorArea must not be negative.' )


   ! TlcdYVerArea - TlcdY initial or fixed vertical cross-section area.

CALL ReadRVar ( UnIn, PriFile, TlcdYVerArea, 'TlcdYVerArea', 'TlcdY initial or fixed vertical cross-section area' )

IF ( TlcdYVerArea < 0.0 )  CALL ProgAbort ( ' TlcdYVerArea must not be negative.' )



   ! TlcdYLen - TlcdY initial or fixed total length.

CALL ReadRVar ( UnIn, PriFile, TlcdYLen, 'TlcdYLen', 'TlcdY initial or fixed total length' )

IF ( TlcdYLen < 0.0 )  CALL ProgAbort ( ' TlcdYLen must not be negative.' )


   ! TlcdYHorLen - TlcdY initial or fixed horizontal length.

CALL ReadRVar ( UnIn, PriFile, TlcdYHorLen, 'TlcdYHorLen', 'TlcdY initial or fixed horizontal length' )

IF ( TlcdYHorLen < 0.0 )  CALL ProgAbort ( ' TlcdYHorLen must not be negative.' )
IF ( TlcdYHorLen > TlcdYLen )  CALL ProgAbort ( ' TlcdYHorLen must be shorter than TlcdYLen.' )


   ! TlcdYHdLoss - TlcdY Initial or fixed head-loss coefficient.

CALL ReadRVar ( UnIn, PriFile, TlcdYHdLoss, 'TlcdYHdLoss', 'TlcdY Initial or fixed head-loss coefficient' )
IF ( TlcdYHdLoss < 0.0 )  CALL ProgAbort ( 'TlcdYHdLoss must not be negative.' )


 ! TlcdYSSpr - Initial TlcdY stop spring stiffness.

CALL ReadRVar ( UnIn, PriFile, TlcdYSSpr, 'TlcdYSSpr', 'TlcdY initial or fixed stop spring stiffness' )

IF ( TlcdYSSpr < 0.0 )  CALL ProgAbort ( ' TlcdYSSpr must not be negative.' )

   ! TlcdYSDamp - Initial TlcdY stop damping.

CALL ReadRVar ( UnIn, PriFile, TlcdYSDamp, 'TlcdYSDamp', 'TlcdY initial or fixed stop damping' )

IF ( TlcdYSDamp < 0.0 )  CALL ProgAbort ( ' TlcdYSDamp must not be negative.' )
...
!yus Start of proposed change.  v7.01  1-Dec-2014.
!yus Number of DOFs increases due to two Tlcds
IF ( NumBl == 2 )  THEN
!remove6.10a   NDOF = 22
   NDOF = 24
ELSE
...
!yus Start of proposed change.  v7.01  1-Dec-2014.
!yus initialize some Tlcd parameters
TlcdXDspInit = TlcdXDsp

TlcdYDspInit = TlcdYDsp

TlcdXHdLossInit = TlcdXHdLoss
TlcdYHdLossInit = TlcdYHdLoss
...
!yus Start of proposed change.  v7.01  1-Dec-2014.
!yus Also note Tlcd DOFs if enabled.

IF ( TlcdXDOF )  THEN
   WRITE (UnSu,FmtTxt)  ' Enabled    TlcdX DOF.'
ELSE
   WRITE (UnSu,FmtTxt)  ' Disabled   TlcdX DOF.'
ENDIF

IF ( TlcdYDOF )  THEN
   WRITE (UnSu,FmtTxt)  ' Enabled    TlcdY DOF.'
ELSE
   WRITE (UnSu,FmtTxt)  ' Disabled   TlcdY DOF.'
ENDIF
